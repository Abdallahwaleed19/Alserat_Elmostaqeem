...
const DailyHadith = () => {
    ...
    const handleShare = async () => {
        ...
        try {
            ...
            if (window.Capacitor && window.Capacitor.isNativePlatform()) {
                await shareImageDataUrl(dataUrl, 'Daily Hadith', 'Share Hadith');
            } else {
    ...
    const sourceLabel = loading ? '...' : (lang === 'ar' ? hadith.source : hadith.sourceEn);
        ...
        <>
            ...
            >
                ...
                <div className="share-card-header">
                    <h2 className="share-card-title">{lang === 'ar' ? 'حديث اليوم' : 'Daily Hadith'}</h2>
                    <p className="share-card-subtitle">{sourceLabel}</p>
                </div>
                <p className="share-card-body share-card-body-ar">
                    {loading ? (lang === 'ar' ? 'جاري التحميل...' : 'Loading...') : `"${hadith.text}"`}
                </p>
                <p className="share-card-meta">{sourceLabel}</p>
