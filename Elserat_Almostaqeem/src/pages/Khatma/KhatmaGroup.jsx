...
const KhatmaGroup = () => {
    ...
    useEffect(() => {
        const stored = localStorage.getItem('zad_khatmas_group');
        if (stored) {
            try {
    ...
    const saveKhatmas = (updated) => {
        setKhatmas(updated);
        localStorage.setItem('zad_khatmas_group', JSON.stringify(updated));
    };
        ...
        return (
            <div className="container animate-slide-down" style={{ paddingTop: '2rem', paddingBottom: '3rem' }}>
                ...
                <div className="card" style={{ marginBottom: '2rem', textAlign: 'center', padding: '2rem', borderTop: '4px solid var(--color-primary)' }}>
                    ...
                    <h2 style={{ fontSize: '1.8rem', marginBottom: '0.5rem' }}>{lang === 'ar' ? 'ختمة' : 'Group Khatma:'}</h2>
                    <h1 style={{ fontSize: '2.5rem', color: 'var(--color-primary)', margin: '0 0 1rem 0' }}>{currentKhatma.name}</h1>
    ...
    return (
        <div className="container animate-slide-down" style={{ paddingTop: '2rem', paddingBottom: '3rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '2rem' }}>
                ...
                <h1 style={{ margin: 0, fontSize: '1.75rem' }}>{lang === 'ar' ? 'ختمة جماعية' : 'Group Khatma'}</h1>
            </div>

            {showCreate ? (
                <form onSubmit={handleCreate} className="card" style={{ padding: '2rem', marginBottom: '2rem' }}>
                    <h3 style={{ marginTop: 0 }}>{lang === 'ar' ? 'إنشاء ختمة جماعية' : 'Create Group Khatma'}</h3>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginTop: '1.5rem' }}>
                        <label style={{ fontSize: '0.9rem', fontWeight: 'bold' }}>{lang === 'ar' ? 'اسم الختمة (مثال: ختمة رمضان الكبرى)' : 'Khatma Name (e.g. Ramadan Group)'}</label>
                        <input
                            type="text"
                            value={newName}
                ...
                <button onClick={() => setShowCreate(true)} className="card" style={{ width: '100%', padding: '2rem', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1rem', border: '2px dashed var(--color-primary)', background: 'var(--color-primary-10)', cursor: 'pointer', marginBottom: '2rem' }}>
                    ...
                    <span style={{ fontSize: '1.2rem', fontWeight: 'bold', color: 'var(--color-primary)' }}>{lang === 'ar' ? 'إنشاء ختمة جماعية' : 'Create Group Khatma'}</span>
                </button>
            )}
